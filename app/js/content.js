...
function scrape() {
  ...
  fetch(endpoint, {
    ...
    .then((data) => {
      ...
      // Function to check if a node represents a login popup
      function isLoginPopup(node) {
        // Customize this function based on the criteria for login popups
        return node.classList.contains('JFPqaw');
      }

      // Function to highlight a login popup
      ...
      const observer = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
          if (mutation.type === 'childList') {
            // Check for added nodes and highlight login popups
            mutation.addedNodes.forEach((node) => {
              // Ensure it's an element node
              if (node.nodeType === Node.ELEMENT_NODE && isLoginPopup(node)) {
                // Highlight the login popup
                highlight(node);
              }
            });
          }
